...
| **Method** | **Endpoint**                   | **Description**                                                               |
| ---------- | ------------------------------ | ----------------------------------------------------------------------------- |
| **POST**   | `/api/v1/games/{code}/start`   | Transitions room state to `IN_PROGRESS`. Generates the 5 locations.           |
| **GET**    | `/api/v1/games/{code}/round`   | Get current round info (game ID and map, but **not** the coordinates).        |
| **POST**   | `/api/v1/games/{code}/guess`   | **The Producer:** Accepts (X, Y, Z) guess. This sends a message to **Kafka**. |
