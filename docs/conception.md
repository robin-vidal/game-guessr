...
| -------------------------------- | ------------------------------------------- | ------------------------------------------------- | --------------------------------- |
| **Phase 1: The Game**            | Identify the title (e.g., _Super Mario 64_) | Search bar with autocompletion                    | Binary (Correct/Incorrect)        |
...
| **Method** | **Endpoint**                   | **Description**                                                    |
| ---------- | ------------------------------ | ------------------------------------------------------------------ |
| **POST**   | `/api/v1/games/{code}/start`   | Transitions state to `IN_PROGRESS`. Generates the 5 locations.     |
| **GET**    | `/api/v1/games/{code}/round`   | Get current round info (game ID and map, but **not** coordinates). |
| **POST**   | `/api/v1/games/{code}/guess`   | **Producer:** Accepts (X, Y, Z) guess. Sends a message to Kafka.   |
| **GET**    | `/api/v1/games/{code}/results` | Returns final scores for all 5 rounds after the game ends.         |
